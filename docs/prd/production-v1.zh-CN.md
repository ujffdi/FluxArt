...
基于当前 Next.js、React 和 TypeScript 应用构建 FluxArt Production V1。将内存仓储替换为由 MySQL 支撑的 Prisma 适配器，增加用户自填用户名/密码账号，实现服务端 session，让积分具备账本支撑，把源图和生成图存储到公开 MinIO bucket，通过可替换的模型供应商 runner 执行图片生成，并根据已验证的 Epay 支付通知履约积分包。
...
7. 作为免费用户，我希望清楚看到生成和历史记录限制，这样能理解为什么可能需要购买积分。
8. 作为积分包用户，我希望购买固定积分包，这样可以为偶尔的生成工作付费。
9. 作为用户，我希望生成前先冻结积分，并在没有可用输出时退回积分，这样失败任务不会不公平地消耗余额。
10. 作为用户，我希望图片任务显示稳定状态，这样能知道生成处于排队、运行、存储、审核、成功、失败或已退款状态。
11. 作为用户，我希望安全上传源图和蒙版，这样编辑和扩图不会因为输入损坏而失败。
12. 作为用户，我希望生成资产被可靠存储，这样历史记录和下载不依赖服务端内存。
13. 作为运营者，我希望图片供应商抽象在统一 runner 后面，这样 OpenAI 可以作为默认供应商，同时保留其他供应商的可能性。
14. 作为运营者，我希望同步和异步供应商被归一化，这样供应商特定响应形态不会泄漏到产品逻辑里。
15. 作为运营者，我希望支付回调经过验证且幂等，这样积分不会被重复发放。
16. 作为运营者，我希望订单履约失败可重试，这样付费用户可以被补偿，而不需要手动改数据库。
17. 作为运营者，我希望积分和支付账本不可变，这样可以支持账单客服和对账。
18. 作为运营者，我希望使用公开 MinIO URL 和不可猜测的 key，这样 V1 交付简单，同时资产不容易被枚举。
19. 作为运营者，我希望校验 MySQL、MinIO、OpenAI、自定义供应商和 Epay 环境配置，这样部署错误能尽早暴露。
...
- 优先消耗即将过期的 Promotional Credits，然后消耗 Purchased Credits。
- 创建图片任务前先冻结积分。只有可用输出通过审核时，才把冻结转换为最终消费。系统失败或输出审核失败时释放或退款冻结积分。
- 使用固定 V1 成本：文生图每张 10 credits，图生图每张 15 credits，局部重绘每次 20 credits，扩图每次 30 credits，高清无水印下载每张 5 credits。
...
- 付费订单必须事务性履约。积分包订单创建 Purchased Credit Buckets 和 Credit Ledger Entries。
...
- `CreditBucket`：用户、来源类型、积分类型、原始金额、剩余金额、有效期窗口、优先级和可选来源订单。
...
- `DownloadEvent`：资产、用户、下载类型、积分成本和时间戳。
